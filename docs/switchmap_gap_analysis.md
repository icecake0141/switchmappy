...
| Operational switchport evidence | Implemented | Cisco-like SSH captures mode, access VLAN, voice VLAN, native VLAN, allowed VLANs, speed, and media/type labels. FortiSwitch SSH captures VLAN membership plus `show switch interface` description, mode, native VLAN, allowed VLANs, and FortiLink hints. SNMP exposes speed and IF-MIB interface type. | Add transceiver-detail fixtures where vendors expose richer optic diagnostics. |
